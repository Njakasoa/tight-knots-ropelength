...
def parse_vect(path):
    ...
    tail=tokens[4+2*m+3*v:]
    if len(tail) not in {3*colors,4*colors}:raise ValueError('unexpected trailing VECT tokens or incomplete color rows')
    for token in tail:Fraction(token)
