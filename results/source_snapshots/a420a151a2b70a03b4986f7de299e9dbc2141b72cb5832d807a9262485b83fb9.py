...
def sampling_proof(points,counts,parameters):
    ...
    for ci in range(12):
        ...
        for k in range(48):
            xyz=list(points[ci*48+k]);xyz[2]/=zs
    ...
    emax=max(bounds,key=lambda x:float(x));separations=[ri,A(2),A(2)-ri,2*ri*(arb.pi()/2).sin(),4*(arb.pi()/3).sin()]
...
class PathCheck:
    def __init__(self,initial,final,counts,alignment):
        ...
        self.cache={(0,0):self.start,(1,0):self.end};self.next=[];self.previous=[];self.component=[];base=0
...
def coverage(rows,required,key_columns):
    intervals={}
    for row in rows:
        key=tuple(int(x) for x in row[:key_columns]);num,depth=map(int,row[key_columns:key_columns+2]);assert depth>=0 and 0<=num<2**depth
